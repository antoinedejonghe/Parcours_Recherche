...
simulations = zeros(NbNoeudsMax,MonteCarlo);
    ...
    for it = 1:MonteCarlo
        ...
        for i =1:NbBS
            ...
            for j=1:n
                ...
                % on compare le SINR par rapport à un certain seuil (6.8 dB
                % car on obtient un BER de 10^-3 avec ce SINR aevc une BPSK
                if ( 6.8 > (10*(log10(orderedPowers(i,j)/(interference+N)))))
                    SIC = 1;
                end
            end
        end
        simulations(n,it) = SIC;
    end
end

for n = 1:NbNoeudsMax
    % calcul du taux de SIC complets
    results(n) = (MonteCarlo-sum(simulations(n,:)))/MonteCarlo;
end

plot(results,'-o');
title('probability of perfect SIC completed vs number of active nodes for 1 BS');
legend('NbBS = 1');
xlabel('number of active nodes');
ylabel('probability of perfect SIC completed');
